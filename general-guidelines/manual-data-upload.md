...
2. LogicApp will trigger once the file is uploaded and copy the file in ADLS raw zone
3. LogicApp will archive the file in an "Archive" folder (in SharePoint)
4. Manual upload ADF is triggered once file is dropped in ADLS raw zone
5. ADF converts the excel file into a csv file
6. This csv file is then placed into ADLS landing zone
7. After dropping a file in the ADLS landing zone, the snowflake copy command would trigger, and the CSV file would load into the final snowflake tables.
...
2. Create the file directory in ADLS raw zone and upload the file to SharePoint. This is where the manual uploaded files will be dropped by the LogicApp.\
...
4.  Make respective new entries in Snowflake metadata table. \
